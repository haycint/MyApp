...
def download_glove(force=False):
    """
    Download GloVe 6B embeddings from Stanford NLP if not already present.
    ...
    if not os.path.exists(zip_path) or force:
        print(f"[GloVe] Downloading GloVe 6B embeddings from {GLOVE_URL}...")
        ...
        try:
            ...
            print("[GloVe] Please download manually from:")
            print(f"        {GLOVE_URL}")
...
def train_one_epoch(model, train_loader, criterion_hm, criterion_ce,
    ...
    Args:
        ...
        criterion_hm: HM_Loss for affordance segmentation
        criterion_ce: CrossEntropyLoss for classification
        optimizer: optimizer
    ...
    Returns:
        mean_loss: average training loss for the epoch
    """
    model.train()
    ...
    for i, (img, points, labels, logits_labels, sub_box, obj_box) in enumerate(train_loader):
        ...
        for point, label, logits_label in zip(points, labels, logits_labels):
            ...
            # Get text embedding for each sample in the batch
            # logits_label contains the affordance class index
            text_emb = affordance_emb_tensor[logits_label]  # [B, text_dim]
        ...
        if i % 10 == 0:
            print(f"  Batch {i}/{num_batches} | Loss: {temp_loss.item():.4f}")
