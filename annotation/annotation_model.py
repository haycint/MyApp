...
架构设计：
- Backbone: ResNet-50 特征提取（支持 ImageNet 预训练）
- FPN: 特征金字塔网络，多尺度特征融合
- RPN: 区域建议网络，生成候选区域
...
预训练权重支持：
- pretrained=True:  使用 torchvision 内置的 ImageNet 预训练权重（自动下载）
- pretrained=False: 随机初始化
- img_model_path:   从本地 .pth 文件加载预训练权重
"""
...
class AnnotationModel(nn.Module):
    ...
    def __init__(self, num_interactions=17, pretrained=True, img_model_path=None):
        """
        Args:
            num_interactions: 可供性类别数量
            pretrained: 是否使用 ImageNet 预训练权重
                        True: 使用 torchvision 内置的 ImageNet1K-V2 权重（自动下载）
                        False: 随机初始化
            img_model_path: 本地预训练权重文件路径 (.pth)
                            如果提供，优先从本地文件加载
        """
        super().__init__()
        ...
        if img_model_path and os.path.exists(img_model_path):
            # 从本地文件加载预训练权重
            print(f"[AnnotationModel] Loading pretrained weights from: {img_model_path}")
            resnet = models.resnet50(weights=None)
            pretrain_dict = torch.load(img_model_path, map_location='cpu', weights_only=True)
            # 处理不同的state_dict格式
            if isinstance(pretrain_dict, dict) and 'state_dict' in pretrain_dict:
                pretrain_dict = pretrain_dict['state_dict']
            resnet.load_state_dict(pretrain_dict, strict=False)
            print(f"[AnnotationModel] Loaded local pretrained ResNet-50")
        elif pretrained:
            # 使用 torchvision 内置的 ImageNet 预训练权重（自动下载）
            print("[AnnotationModel] Using ImageNet pretrained ResNet-50 (auto-download via torchvision)")
            resnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2)
            print("[AnnotationModel] Loaded ImageNet pretrained ResNet-50 successfully")
        else:
            # 随机初始化
            print("[AnnotationModel] Using randomly initialized ResNet-50 (no pretrained weights)")
            resnet = models.resnet50(weights=None)
    ...
    def _generate_proposals(self, rpn_cls, rpn_bbox, feature_size,
                            pre_nms_top_n=1000, post_nms_top_n=300):
        """生成候选区域"""
        ...
        for i in range(batch_size):
            ...
            # 裁剪到图像范围
            proposals_i = torch.clamp(proposals_i, 0, 224)
    ...
    def _post_process(self, proposals, cls_scores, bbox_preds,
                      image_size, device, conf_threshold=0.5):
        """后处理：NMS和结果整理"""
        results = {
            'subject_boxes': [],
            'object_boxes': [],
        ...
        for batch_idx, props in enumerate(proposals):
            ...
            for cls_idx, cls_name in [(1, 'subject'), (2, 'object')]:
                ...
                results['all_labels'].append(
                    torch.full((len(final_boxes),), cls_idx, device=device)
                )

            offset += n_props

        # 合并结果
        for key in ['subject_boxes', 'object_boxes',
                     'subject_scores', 'object_scores']:
            if results[key]:
                results[key] = torch.cat(results[key], dim=0)
            else:
                results[key] = (
                    torch.tensor([], device=device).reshape(0, 4)
                    if 'box' in key
...
def build_annotation_model(num_interactions=17, pretrained=True, img_model_path=None):
    """
    构建标注模型
    ...
    Args:
        num_interactions: 可供性类别数量
        pretrained: 是否使用 ImageNet 预训练权重
                    True: 自动通过 torchvision 下载 ImageNet 预训练权重
                    False: 随机初始化
        img_model_path: 本地预训练权重文件路径
                        如果提供，优先从本地文件加载
    """
...
class AnnotationLoss(nn.Module):
    ...
    def __init__(self, loss_weights=None):
        """
        Args:
            loss_weights: 各损失项的权重字典，默认全部为1.0
        """
        super().__init__()
    ...
    def forward(self, outputs, targets):
        ...
        if 'rpn_cls' in outputs and 'rpn_targets' in targets:
            ...
            if valid.sum() > 0:
                losses['rpn_cls_loss'] = (
                    self.loss_weights.get('rpn_cls', 1.0) *
                    self.rpn_cls_loss(rpn_cls[valid], rpn_targets[valid].float())
                )
        ...
        if 'cls_scores' in outputs and 'gt_labels' in targets:
            ...
            if len(gt_labels) > 0:
                losses['cls_loss'] = (
                    self.loss_weights.get('cls', 1.0) *
                    self.cls_loss(cls_scores, gt_labels)
                )
        ...
        if 'bbox_preds' in outputs and 'gt_boxes' in targets and 'gt_labels' in targets:
            ...
            if fg_mask.sum() > 0:
                losses['bbox_loss'] = (
                    self.loss_weights.get('bbox', 1.0) *
                    self.bbox_loss(bbox_preds[fg_mask], gt_boxes[fg_mask])
                )
            ...
            if 'gt_interactions' in targets:
                ...
                if len(gt_interactions) > 0:
                    losses['interaction_loss'] = (
                        self.loss_weights.get('interaction', 1.0) *
                        self.interaction_loss(interaction_logits, gt_interactions)
                    )
